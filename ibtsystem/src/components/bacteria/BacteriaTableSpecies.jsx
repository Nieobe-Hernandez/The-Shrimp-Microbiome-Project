...
const BacteriaTableSpecies = ({ apiUrl, savedState, onStateChange }) => {
  ...
  useEffect(() => {
    if (!savedState.data.length) {
      fetchData(0, 198865, savedState.searchQuery);
    } else {
  ...
  return (
    <>
      <Autocomplete
        ...
        renderInput={(params) => <TextField {...params} label="Search by Species" />}
      ...
      <PrevalenceTable selectedBacteria={selectedValue ? selectedValue.label : ''} nivel="Species" />
    </>
  );
};

export default BacteriaTableSpecies;
