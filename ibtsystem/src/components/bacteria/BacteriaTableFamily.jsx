...
const BacteriaTableFamily = ({ apiUrl, savedState, onStateChange }) => {
  ...
  useEffect(() => {
    if (!savedState.data.length) {
      fetchData(0, 86180, savedState.searchQuery);
    } else {
  ...
  return (
    <>
      <Autocomplete
        ...
        renderInput={(params) => <TextField {...params} label="Search by Family" />}
      ...
      <PrevalenceTable selectedBacteria={selectedValue ? selectedValue.label : ''} nivel="Family" />
    </>
  );
};

export default BacteriaTableFamily;
