...
const BacteriaTableOrder = ({ apiUrl, savedState, onStateChange }) => {
  ...
  useEffect(() => {
    if (!savedState.data.length) {
      fetchData(0, 49135, savedState.searchQuery);
    } else {
  ...
  return (
    <>
      <Autocomplete
        ...
        renderInput={(params) => <TextField {...params} label="Search by Order" />}
 ...
 <Box >
        ...
        </Box>      <PrevalenceTable selectedBacteria={selectedValue ? selectedValue.label : ''} nivel="Order" />
    </>
  );
};

export default BacteriaTableOrder;
