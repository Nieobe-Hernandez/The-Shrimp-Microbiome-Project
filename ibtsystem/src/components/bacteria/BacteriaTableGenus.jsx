...
const BacteriaTableGenus = ({ apiUrl, savedState, onStateChange }) => {
  ...
  useEffect(() => {
    if (!savedState.data.length) {
      fetchData(0, 173445, savedState.searchQuery);
    } else {
  ...
  return (
    <>
      <Autocomplete
        ...
        renderInput={(params) => <TextField {...params} label="Search by Genus" />}
      ...
      <PrevalenceTable selectedBacteria={selectedValue ? selectedValue.label : ''} nivel="Genus" />
    </>
  );
};

export default BacteriaTableGenus;
