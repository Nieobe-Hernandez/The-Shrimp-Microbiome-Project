...
const BacteriaTableKingdom = ({ apiUrl, savedState, onStateChange }) => {
  ...
  useEffect(() => {
    if (!savedState.data.length) {
      fetchData(0, 620, savedState.searchQuery);
    } else {
  ...
  return (
    <>
      <Autocomplete
        ...
        renderInput={(params) => <TextField {...params} label="Search by Kingdom" />}
      ...
      <PrevalenceTable selectedBacteria={selectedValue ? selectedValue.label : ''} nivel="Kingdom" />
    </>
  );
};

export default BacteriaTableKingdom;
