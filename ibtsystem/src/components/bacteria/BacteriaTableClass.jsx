...
const BacteriaTableClass = ({ apiUrl, savedState, onStateChange }) => {
  ...
  useEffect(() => {
    if (!savedState.data.length) {
      fetchData(0, 20305, savedState.searchQuery);
    } else {
  ...
  return (
    <>
      <Autocomplete
        ...
        renderInput={(params) => <TextField {...params} label="Search by Class" />}
      ...
      <PrevalenceTable selectedBacteria={selectedValue ? selectedValue.label : ''} nivel="Class" />
    </>
  );
};

export default BacteriaTableClass;
